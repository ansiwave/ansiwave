...
proc tick*(session: var EditorSession, ctx: var context.Context, rawInput: tuple[key: iw.Key, codepoint: uint32], focused: bool) =
  ...
  # render top bar
  if globals.midiProgress == nil:
    proc topBarView(ctx: var context.Context, data: ref context.State, node: JsonNode, children: seq[JsonNode]) =
    ...
    proc midiProgressView(ctx: var context.Context, data: ref context.State, node: JsonNode, children: seq[JsonNode]) =
      ctx = nimwave.slice(ctx, 0, 0, iw.width(ctx.tb), if bufferId == Editor: 2 else: 1)
  ...
  proc bufferView(ctx: var context.Context, data: ref context.State, node: JsonNode, children: seq[JsonNode]) =
    let
      parentHeight =
        if ctx.parent == nil:
          iw.height(ctx.tb)
  ...
  # render bottom bar
  proc bottomBarView(ctx: var context.Context, data: ref context.State, node: JsonNode, children: seq[JsonNode]) =
